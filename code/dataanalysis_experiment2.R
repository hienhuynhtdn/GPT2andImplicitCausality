...
df <- read.delim("./data/experiment2_textgeneration.csv", sep = ",",
...
summary <- df_1 %>% 
  group_by(bias,subject_remention) %>%
  tally %>%
  group_by(bias) %>%
  mutate(pct = n/sum(n))
...
summary_2 <- df_2 %>% 
  group_by(bias,subject_remention,subject_gender) %>%
  tally %>%
...
plot_1 <- ggplot(summary, aes(x=bias, y=n, fill=as.factor(subject_remention))) +
...
plot_2 <- ggplot(summary_2, aes(x=bias, y=n, fill=as.factor(subject_remention))) +
  geom_bar(stat="identity") +
  geom_text(aes(label=paste0(sprintf("%1.1f", pct*100),"%")), 
